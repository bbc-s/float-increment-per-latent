...
- `dynamic_adapter_policy`: controls which adapter types are changed per sample
...
- `lora_only_fast`: apply all adapters statically at base strength, then change only standard LoRA adapters per sample.
- `exact_all_adapters`: change supported LoRA and direct LoKr adapters per sample, but this can be too slow or memory-heavy for large stacks on 8 GB GPUs.
